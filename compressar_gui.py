...
class CompressarGui():
    def __init__(self):
        ...
        # select image button
        self.select_button = tk.Button(self.window, text="Select Image", bg='orange', command=self.select_image)
        self.select_button.grid(row=0, column=0, sticky='w', padx=5, pady=10)

        # Property label
        self.property_label = tk.Label(self.window, text='New Property:', bg='orange')
        self.property_label.grid(row=1, column=0, sticky='w', padx=5, pady=5)

        # Scale widget to adjust the quality of the image
        self.quality_scale = Scale(self.window, from_=1, to=100, orient='horizontal', label="Select Quality:", bg="orange")
        self.quality_scale.set(90)
        self.quality_scale.grid(row=2, sticky='nsew', padx=50, pady=5,columnspan=2)

        # Destination folder selector
        self.destination_label = tk.Label(self.window, text='Destination:', bg='orange')
        self.destination_label.grid(row=3, column=0, sticky='w', padx=5, pady=5)

        self.destination_btn = tk.Button(self.window, text="Select Folder", command=self.destination_folder)
        self.destination_btn.grid(row=4, column=0, sticky='nse', padx=15, pady=5)

        self.entry_var = tk.StringVar()
        self.destination_entry = tk.Entry(self.window, textvariable=self.entry_var)
        ...
        self.to_jpg_checkbutton = Checkbutton(self.window, text="Convert to JPEG", variable=self.to_jpg_var, bg='orange')
        self.to_jpg_checkbutton.grid(row=6, sticky='w', padx=30, pady=5)

        # fact button
        self.facts_btn = tk.Button(self.window, text='Quick Fact', command=self.display_facts)
        self.facts_btn.grid(row=6, column=2, columnspan=2, sticky="swe", padx=5, pady=5)
        
        # compress button
        self.compressbtn = tk.Button(self.window, text='compress', command=self.compress)
    ...
    def select_image(self):
        self.file_path = askopenfilename(title="Select Image", filetypes=[("Image Files", "*.jpg *.jpeg *.png")])
        if self.file_path:
    ...
    def compress(self):
        ...
        if self.to_jpg_value and ext not in ['.jpg', '.jpeg']:
            ...
            img.save(f'{self.folder_path}/{self.image_name}', "JPEG", optimize=True, quality=self.slider_value)
        else:
            maximum_width = 1200
            img = Image.open(self.file_path)
            ...
            if not self.folder_path:
                user_desktop = os.path.expanduser("~/Desktop")
                self.folder_path = user_desktop
            img.save(f'{self.folder_path}/{self.image_name}', optimize=True, quality=self.slider_value)
